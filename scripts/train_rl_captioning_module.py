...
def train_rl_cap(cfg):
    ...
    for epoch in range(cfg.epoch_num):
        ...
        if epoch % 2 == 0 and epoch < cfg.one_by_one_starts_at:
            try:
        ...
        if epoch >= cfg.one_by_one_starts_at:
            ...
            for i, val_loader in enumerate(val_loaders):
                metrics_avg[i] = eval_model(cfg, model, val_loader, greedy_decoder, epoch, TBoard)

                log_prefix = f"{log_prefix}@{metrics_avg[i]['METEOR'] * 100}"

            if best_metric < metrics_avg[0]['METEOR']:
                best_metric = metrics_avg[0]['METEOR']
                try:
